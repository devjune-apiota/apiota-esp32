/*
  ================================================================
   MultiTaskOTA_WiFiManager — APIOTA Library Example
   ESP32 Dev Module (no display) — OTA with FreeRTOS multitasking
                                   + WiFiManager captive portal
   ────────────────────────────────────────────────
   Like MultiTaskOTA (no delay(), OTA in its own task), but WiFi is
   configured through a captive portal instead of hardcoded
   credentials. On first boot (or when no known WiFi is found) the
   device opens an access point "APIOTA-ESP32-SETUP" — connect to it
   with a phone and pick your network.
...
#define DEVICE_NAME        "MultiTaskESP32"
